...
namespace gsparc
{
    CommandLineOptions::CommandLineOptions()
        : short_opt("X:Y:c:a:m:x:y:z:n:g:d:i:m:qbhv")
    {
    ...
    void CommandLineOptions::Initialize()
    {
        ...
        this->_idxmatch_mode = 0;    // Default: SLITOM
        ...
        const struct option long_opt[] = {
            ...
            {"contract-mode", required_argument, 0, 'c'},
            {"add-contract-mode", required_argument, 0, 'a'},
            {"multi-sptc", required_argument, 0, 't'},\
            {"m", required_argument, 0, 'm'},
            {"x", required_argument, 0, 'x'},
            {"y", required_argument, 0, 'y'},
            {"z", required_argument, 0, 'z'},
    ...
    void CommandLineOptions::Parse(int argc, char **argv)
    {
        ...
        while ((c = getopt_long(argc, argv, this->short_opt, this->long_opt, NULL)) != -1)
        {
            ...
            case 't':
                ...
                break;
            case 'm':
                this->_memory = (int)atoll(optarg);
                fprintf(stderr, "memory: %d GB\n", this->_memory);
                break;
            case 'n':
            ...
            case 'd':
                ...
                break;
            case 'i':
                this->_idxmatch_mode = (int)atoll(optarg);
                fprintf(stderr, "idxmatch_mode: %d (0=SLITOM, 1=COO, 2=BLCO)\n", this->_idxmatch_mode);
                break;
            case 'q':
                this->_qunatum = true;
                break;
