...
namespace common
{
// [local-build] nvcc compat: immintrin.h is blocked in main.cu for nvcc,
// so provide _pdep_u64/_pext_u64 via inline asm when compiled by nvcc.
#ifdef __CUDACC__
static __host__ inline uint64_t _pdep_u64(uint64_t val, uint64_t mask) {
    uint64_t result;
    asm("pdep %2, %1, %0" : "=r"(result) : "r"(val), "r"(mask));
    return result;
}
static __host__ inline uint64_t _pext_u64(uint64_t val, uint64_t mask) {
    uint64_t result;
    asm("pext %2, %1, %0" : "=r"(result) : "r"(val), "r"(mask));
    return result;
}
#else
#include <immintrin.h>   // original
#endif
