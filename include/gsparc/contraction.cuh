...
namespace gsparc
{
    ...
    __global__ void contraction_dense_kernel(LIndexType *X_indices,
                                             ValueType *X_values,
                                             ...
                                             int cbits)
    {
        ...
        {
            index_t yidx = mPos[tid];
            index_t cnt = mCnt[tid];
            for (int offset = 0; offset < cnt; ++offset)
            {
                lindex_t x_fidx = (X_indices[tid] >> cbits);
                lindex_t y_fidx = (Y_indices[yidx + offset] & ((static_cast<lindex_t>(1) << Y_nfbits) - 1));

                lindex_t result_idx = (x_fidx << X_nfbits) + y_fidx;
    ...
    template <typename IndexType, typename LIndexType, typename ValueType>
    __global__ void contraction_esc_kernel(LIndexType *X_indices,
                                           ValueType *X_values,
                                           LIndexType *Y_indices,
                                           ValueType *Y_values,
                                           uint64_t X_nnz,
                                           ...
                                           int cbits)
    {
        ...
        for (index_t idx = tid; idx < ir_nnz; idx += stride)
        {
            ...
            lindex_t y_fidx = Y_indices[yidx + offset] & ((static_cast<index_t>(1) << Y_fbits) - 1);
    ...
    template <typename IndexType, typename LIndexType, typename ValueType>
    void Dynamic_partition(LIndexType *X_indices,
                           uint64_t X_nnz,
                           uint64_t ir_nnz,
                           IndexType *mCntPrefix,
                           int ncbits,
                           ...
                           uint64_t *max_prtn_ir_nnz)
    {
        ...
        while (start_pointer < X_nnz)
        {
            ...
            while (true)
            {
                ...
                while (candidate < X_nnz - 1 &&
                       ((X_indices[candidate] >> ncbits) == (X_indices[candidate + 1] >> ncbits)))
                {
                    candidate++;
                }
    ...
    template <typename IndexType, typename LIndexType, typename ValueType>
    void Dense_Contraction(LIndexType *X_indices,
                           ValueType *X_values,
                           IndexType X_nnz,
                           LIndexType *Y_indices,
                           ValueType *Y_values,
                           ...
                           Timer *timer)
    {
        ...
        printf("X_nnz: %lu, Y_nnz: %lu, ir_size: %lu, dense_start: %lu, dense_size: %lu\n", X_nnz, Y_nnz, ir_size, dense_start, dense_size);
        timer->start();
        ...
        gsparc::contraction_dense_kernel<index_t, lindex_t, value_t><<<blocks_per_grid, threads_per_block>>>(X_indices,
                                                                                                             X_values,
                                                                                                             Y_indices,
                                                                                                             Y_values,
                                                                                                             X_nnz,
        ...
        // // cub sort
        // common::cuda::end_timer_with_msg(&timer, &stop, "contraction_dense_kernel");
...
#pragma omp parallel for reduction(+ : offset) schedule(static)
    ...
    template <typename IndexType, typename LIndexType, typename ValueType>
    void ESC_Contraction(LIndexType *X_indices,
                         ValueType *X_values,
                         uint64_t X_nnz,
                         LIndexType *Y_indices,
                         ValueType *Y_values,
                         ...
                         Timer *timer)
    {
        ...
        lindex_t **d_Z_indices = static_cast<lindex_t **>(common::cuda::pinned_malloc(sizeof(lindex_t *) * 1));
        value_t **d_Z_values = static_cast<value_t **>(common::cuda::pinned_malloc(sizeof(value_t *) * 1));
        ...
        for (int i = 0; i < num_prtn; ++i)
        {
            ...
            else
            {
                ...
                // sub_X_start = prtn_offset[i];
                // sub_X_end = prtn_offset[i + 1];
                // prtn_nnz = sub_X_end - sub_X_start;
                // prtn_ir_nnz = h_mCntPrefix[sub_X_end] - h_mCntPrefix[sub_X_start];
            }
            printf("sub_X_start: %lu, sub_X_end: %lu, prtn_nnz: %lu, prtn_ir_nnz: %lu\n", sub_X_start, sub_X_end, prtn_nnz, prtn_ir_nnz);
            if (prtn_ir_nnz == 0)
                continue;
            ...
            gsparc::contraction_esc_kernel<index_t, lindex_t, value_t><<<blocks_per_grid, threads_per_block, 0, stream>>>(X_indices + sub_X_start,
            ...
            printf("before allocate d_keys and d_vals\n");
            printf("memory: %s\n", common::byteToString(prtn_ir_nnz * (sizeof(lindex_t) + sizeof(value_t))));
            ...
            printf("do not allocate d_keys and d_vals\n");

            cub::DeviceRadixSort::SortPairs(d_temp_storage, temp_storage_bytes_sort,
                                            d_Z_indices[0], d_Z_indices[0],
                                            d_Z_values[0], d_Z_values[0],
                                            prtn_ir_nnz,
                                            0, 64, stream);
            ...
            printf("temp_storage_bytes: %s\n", common::byteToString(temp_storage_bytes));
            memoryPool->printFree();
            common::cuda::stream_sync(stream);

            d_temp_storage = memoryPool->allocate<void>(temp_storage_bytes);
            printf("allocated d_temp_storage\n");
            ...
            // printf("allocated d_temp_storage\n");
            common::cuda::d2hcpy_async(sub_Z_indices, d_Z_indices[0], sizeof(lindex_t) * temp_result_nnz, stream);
            common::cuda::d2hcpy_async(sub_Z_values, d_Z_values[0], sizeof(value_t) * temp_result_nnz, stream);
            common::cuda::stream_sync(stream);
            // for (int j = 0; j < temp_result_nnz; ++j){
            //     printf("sub_Z_indices[%d]: %lu, sub_Z_values[%d]: %f\n", j, sub_Z_indices[j], j, sub_Z_values[j]);
            // }
        ...
        // common::end_timer_with_msg(&timer, "cub::DeviceRadixSort::SortPairs");
        ...
        //     offset += sub_Z_nnz[i];
        // }
        // common::end_timer_with_msg(&timer, "copy result to Z");
    ...
    template <typename IndexType, typename LIndexType, typename ValueType>
    void ESC_Contraction_multi(LIndexType *X_indices,
                               ValueType *X_values,
                               uint64_t X_nnz,
                               LIndexType *Y_indices,
                               ValueType *Y_values,
                               ...
                               Timer *timer)
    {
        ...
        for (int i = 0; i < num_prtn; ++i)
        {
            ...
            memoryPool->deallocate<index_t>(d_num_segments, 1);
            // printf("allocated d_temp_storage\n");
            common::cuda::d2hcpy_async(sub_Z_indices[i], d_Z_indices[0], sizeof(lindex_t) * temp_result_nnz, stream);
            common::cuda::d2hcpy_async(sub_Z_values[i], d_Z_values[0], sizeof(value_t) * temp_result_nnz, stream);
            common::cuda::stream_sync(stream);
            common::cuda::device_sync();
            sub_Z_nnz[i] = temp_result_nnz;
            *Z_nnz += temp_result_nnz;
