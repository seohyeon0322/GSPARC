...
#include "gsparc/indexMatch_blco.cuh"
...
#pragma omp parallel for num_threads(gpu_count) schedule(static, 1)
        for (int i = 0; i < X_prtn_num; ++i)
        {
            ...
            for (int j = 0; j < Y_prtn_num; j++)
            {
                ...
                if (idxmatch_mode == 1 && SX->coo_cmodes && SY->coo_cmodes) {
                    // COO index matching
                    uint32_t **d_coo_X = new uint32_t*[SX->cnmodes];
                    uint32_t **d_coo_Y = new uint32_t*[SY->cnmodes];
                    for (int c = 0; c < SX->cnmodes; ++c) {
                        d_coo_X[c] = memory_pool->allocate<uint32_t>(sub_X_nnz);
                        common::cuda::h2dcpy_async(d_coo_X[c], SX->coo_cmodes[c] + sub_X_start,
                                                   sub_X_nnz * sizeof(uint32_t), stream1);
                    }
                    for (int c = 0; c < SY->cnmodes; ++c) {
                        d_coo_Y[c] = memory_pool->allocate<uint32_t>(sub_Y_nnz);
                        common::cuda::h2dcpy_async(d_coo_Y[c], SY->coo_cmodes[c] + sub_Y_start,
                                                   sub_Y_nnz * sizeof(uint32_t), stream1);
                    }
                    common::cuda::stream_sync(stream1);
                    gsparc::IndexMatch_COO<index_t>(d_coo_X, d_coo_Y,
                                                     sub_X_nnz, sub_Y_nnz,
                                                     SX->cnmodes,
                                                     d_mPos, d_mCnt,
                                                     d_mCntPrefix, mCntPrefix,
                                                     &ir_nnz, stream1,
                                                     memory_pool, gpu_count);
                    for (int c = 0; c < SX->cnmodes; ++c)
                        memory_pool->deallocate<uint32_t>(d_coo_X[c], sub_X_nnz);
                    for (int c = 0; c < SY->cnmodes; ++c)
                        memory_pool->deallocate<uint32_t>(d_coo_Y[c], sub_Y_nnz);
                    delete[] d_coo_X;
                    delete[] d_coo_Y;
                } else if (idxmatch_mode == 2 && SX->blco_ckeys && SY->blco_ckeys) {
                    // BLCO index matching (full BLCO key with on-the-fly contraction extraction)
                    uint64_t *d_blco_X = memory_pool->allocate<uint64_t>(sub_X_nnz);
                    uint64_t *d_blco_Y = memory_pool->allocate<uint64_t>(sub_Y_nnz);
                    common::cuda::h2dcpy_async(d_blco_X, SX->blco_ckeys + sub_X_start,
                                               sub_X_nnz * sizeof(uint64_t), stream1);
                    common::cuda::h2dcpy_async(d_blco_Y, SY->blco_ckeys + sub_Y_start,
                                               sub_Y_nnz * sizeof(uint64_t), stream1);
                    common::cuda::stream_sync(stream1);
                    gsparc::IndexMatch_BLCO<index_t>(d_blco_X, d_blco_Y,
                                                      sub_X_nnz, sub_Y_nnz,
                                                      d_mPos, d_mCnt,
                                                      d_mCntPrefix, mCntPrefix,
                                                      &ir_nnz, stream1,
                                                      memory_pool, gpu_count,
                                                      SX->cnmodes,
                                                      SX->blco_cmode_offsets,
                                                      SX->blco_cmode_widths);
                    memory_pool->deallocate<uint64_t>(d_blco_X, sub_X_nnz);
                    memory_pool->deallocate<uint64_t>(d_blco_Y, sub_Y_nnz);
                } else {
                    // SLITOM index matching (default)
                ...
                uint64_t findex_range = last_findex - first_findex + 1;
                uint64_t y_fspace = (SY->nfbits < 64) ? (static_cast<uint64_t>(1) << SY->nfbits) : UINT64_MAX;
                bool dense_overflow = (y_fspace != 0 && findex_range > UINT64_MAX / y_fspace);
                uint64_t dense_nnz = dense_overflow ? UINT64_MAX : findex_range * y_fspace;
                lindex_t dense_start = first_findex << SY->nfbits;

                // Total Dense memory: d_dense_values(value_t) + d_Z_indices(lindex_t)
                //   + d_Z_values(value_t) + d_flags(char) + CUB temp
                constexpr uint64_t dense_entry_bytes = sizeof(value_t) + sizeof(lindex_t) + sizeof(value_t) + 1;
                bool total_overflow = dense_overflow || (dense_nnz > UINT64_MAX / dense_entry_bytes);
                uint64_t dense_total_size = total_overflow ? UINT64_MAX : dense_nnz * dense_entry_bytes;
                printf("ir_nnz: %llu, dense_nnz: %llu\n", ir_nnz, dense_nnz);
                printf("esc_size : %s\n", common::byteToString(esc_size));
                printf("dense_total_size : %s\n", common::byteToString(dense_total_size));
                printf("memory_pool->is_available(dense_total_size): %d\n", memory_pool->is_available(dense_total_size));
                memory_pool->printFree();

                // Improved heuristic: prefer Dense when its TOTAL memory footprint
                // (including sparsification buffers) fits and is smaller than ESC
                bool default_dense = memory_pool->is_available(dense_total_size)
                    && (dense_total_size < esc_size   // Dense memory < ESC memory
                        || (dense_nnz < ir_nnz && dense_nnz > sub_X_nnz));

                bool use_dense;
                if (dense_accumulator == 1) {
                use_dense = true;
                if (memory_pool->is_available(dense_total_size) == false){
                    fprintf(stderr, "Out of memory for dense accumulator (need %s)\n",
                            common::byteToString(dense_total_size));
                    exit(1);
                }
                }
...
#pragma omp parallel for num_threads(gpu_count) schedule(static, 1)
        for (int i = 0; i < X_prtn_num; ++i)
        {
            ...
            for (int j = 0; j < Y_prtn_num; j++)
            {
                ...
                uint64_t dense_acc_size = dense_overflow ? UINT64_MAX : dense_nnz * sizeof(value_t);
                printf("ir_nnz: %llu, dense_nnz: %llu\n", ir_nnz, dense_nnz);
                printf("esc_size : %s\n", common::byteToString(esc_size));
                ...
                memory_pool->printFree();
                // Improved heuristic: prefer Dense when its memory footprint is
                // smaller than ESC, or when the original conditions hold
                if (memory_pool->is_available(dense_acc_size)
                    && (dense_acc_size < esc_size
                        || (dense_nnz < ir_nnz && dense_nnz > sub_X_nnz))) /* Dense Accumulator */
                {
                    // Dense
                    fprintf(stderr, "Dense Accumulator\n");
