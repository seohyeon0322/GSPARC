...
#define SLITOM_TEMPLATE_ARGS MaskType, ULIndexType, LIndexType, ValueType
    ...
    public:
        ...
        /* COO/BLCO index matching (ablation) */
        uint32_t **coo_cmodes;    // [cnmodes][nnz] per-mode contraction indices
        uint64_t *blco_ckeys;    // [nnz] linearized keys (ALL modes in natural order)
        int *blco_cmode_offsets;  // [cnmodes] bit offset of each contraction mode in BLCO key
        int *blco_cmode_widths;   // [cnmodes] bit width of each contraction mode in BLCO key
