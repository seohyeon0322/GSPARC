...
const char *const short_opt = "hvi:o:q:b:r:m:x:d:t:s:e:f:cpk:n:b:";
...
int main(int argc, char **argv)
{
	...
	while ((c = getopt_long(argc, argv, short_opt, long_opt, NULL)) != -1)
	{
		...
		case 'i':
			...
			fprintf(stderr, "text_file: %s\n", text_file.c_str());
			printf("text_file: %s\n", text_file.c_str());
			break;
		case 'q':
			quantum_file = std::string(optarg);
			fprintf(stderr, "quantum_file: %s\n", quantum_file.c_str());
			printf("quantum_file: %s\n", quantum_file.c_str());
			break;
		case 'o':
	...
	else if (!quantum_file.empty())
	{
		BEGIN_TIMER(&ticks_start);
		ImportQuantumTensor(quantum_file.c_str(), &X);
		END_TIMER(&ticks_end);
		ELAPSED_TIME(ticks_start, ticks_end, &t_read);
		PRINT_TIMER("Reading text file", t_read);
	}
	...
	if (kernel_id)
	{
		...
		int num_iter = 1;
		if (!stream_data)
			max_block_size = X->nnz;
		void *test;
		// cudaHostAlloc(&test, 400 << 30, cudaHostAllocPortable);
		// test = malloc(200 << 30);
		wtime = 0.0;
		blcotensor *tensor = gen_blcotensor_host<LIType>(X, max_block_size, &wtime);
		printf("warm up: BLCO tensor generation time:   %f\n", wtime);
		IType storage = X->nnz * sizeof(IType) + tensor->block_count * sizeof(IType);
		fprintf(stderr, "BLCO block count: %lld\n", tensor->block_count);
		fprintf(stderr, "BLCO tensor storage: %lld B\n", storage);
		fprintf(stderr, "BLCO tensor storage: %s\n", sptBytesString(storage));
		storage += X->nnz * sizeof(FType);
		fprintf(stderr, "BLCO tensor storage: %s\n", sptBytesString(storage));
		...
		fprintf(stderr, "BLCO tensor generation time:   %f\n\n\n", wtime / num_iter);
		...
		// {
		// 	if (!do_mttkrp_bench)
		// 		printf("Warning: check not implemented for CPD. Checking MTTKRP instead\n");
		...
		// 		mttkrp_alto_dev<IType>(tensor, M, kernel_id, max_iters, target_mode, thread_cf, stream_data, do_batching, num_partitions);
		// 	else
		// 		cpals_blco_dev(tensor, M, max_iters, epsilon, kernel_id, stream_data, do_batching, thread_cf, num_partitions);
		// 	if (text_file_out != "")
		// 	{
