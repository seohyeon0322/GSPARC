...
blcotensor *gen_blcotensor_host(SparseTensor *spt, IType max_block_size, double *time);
...
template <typename LIT>
static inline blcotensor *gen_blcotensor_host(SparseTensor *spt, IType max_block_size, double *time)
{
    double wtime_s, wtime;
    double total_s, total_e;

    // Init ALTO
    total_s = omp_get_wtime();

    AltoTensor<LIT> *_at;
    gen_alto(spt, &_at);
    assert(_at);
    total_e = omp_get_wtime();
    printf("ALTO: Total time = %f (s)\n", total_e - total_s);
    *time += total_e - total_s;
    ...
    check_cuda(cudaMallocHost((void**)&X->coords, sizeof(IType) * nnz), "cudaMallocHost coords"); // Pinned mem
    check_cuda(cudaMallocHost((void**)&X->values, sizeof(FType) * nnz), "cudaMallocHost values");
    total_s = omp_get_wtime();
    for (IType i = 0; i < nmode; i++)
        X->modes[i] = _at->dims[i];
    ...
    int block_count = 0;
    for (int i = 0; i < nmode; i++)
        block_count += (sizeof(IType) * 8) - clz(_at->dims[i] - 1);
    ...
    for (IType block = 0; block < block_count; block++)
    {
        ...
        if (nnz > 0)
        {
            ...
            for (IType stride = 0; stride < nnz; stride += max_block_size)
            {
                ...
                curr_blocks++;
                for (IType i = 0; i < X->N; i++)
                    blk->block_coords[i] = block_coords[i];
            }
        }
    }
...
inline blco_block *gen_block_host(IType N, IType nnz, blcotensor *parent)
{
    ...
    check_cuda(cudaMallocHost((void**)&b->block_coords, sizeof(IType) * N), "cudaMallocHost block_coords");
    b->nmode = N;
    b->nnz = nnz;
    b->parent = parent;
