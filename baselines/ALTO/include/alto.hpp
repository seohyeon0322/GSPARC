...
create_alto(SparseTensor *spt, AltoTensor<LIT> **at, int nprtn)
{
    ...
    // setup_packed_alto(_at, MSB_FIRST, LONG_FIRST);
...
setup_packed_alto(AltoTensor<LIT> *at, PackOrder po, ModeOrder mo)
{
    ...
    { // Dilation & shifting.
        ...
        if (mo == SHORT_FIRST)
            std::sort(mode_bits, mode_bits + nmode, [](auto &a, auto &b)
...
#pragma omp parallel for
    ...
    std::sort(inds, inds + nnz, [at](IType a, IType b)
              { return at->idx[a] < at->idx[b]; });

#pragma omp parallel for
    for (IType i = 0; i < nnz; i++)
        temp_inds[i] = at->idx[inds[i]];
#pragma omp parallel for
    for (IType i = 0; i < nnz; i++)
        at->idx[i] = temp_inds[i];
#pragma omp parallel for
    for (IType i = 0; i < nnz; i++)
        temp_val[i] = at->vals[inds[i]];
#pragma omp parallel for
    for (IType i = 0; i < nnz; i++)
        at->vals[i] = temp_val[i];
