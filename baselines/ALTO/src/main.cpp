...
const char *const short_opt = "hvi:q:o:b:r:m:x:d:t:s:e:f:cp";
...
int main(int argc, char **argv)
{
	...
	else if (!text_file.empty())
	{
		...
		read_sparse_tensor(text_file.c_str(), &X, 1);
		END_TIMER(&ticks_end);
		ELAPSED_TIME(ticks_start, ticks_end, &t_read);
	...
	if (do_mttkrp_bench)
	{
		...
		BenchmarkAlto(X, max_iters, rank, seed, target_mode, 32);
		return 0;
#ifdef memtrace
				   ...
				   IType seed, int target_mode, int num_partitions)
{
	...
	destroy_alto(AT);
	// fprintf(stderr, "warm up ALTO creation time:   %f\n", wtime);

	double alto_create = 0.0;
	int num_iter = 1;
	for (int iter = 0; iter < num_iter; ++iter)
	{
		...
		// fprintf(stderr, "iter %d ALTO creation time:   %f\n", iter, omp_get_wtime() - wtime_s);

		alto_create += omp_get_wtime() - wtime_s;
		destroy_alto(AT);
		std::this_thread::sleep_for(std::chrono::milliseconds(500));
	}
	// printf("ALTO creation time:   %f\n", wtime);
	fprintf(stderr, "ALTO creation time:   %f\n\n\n", alto_create / num_iter);
	printf("\n\n\n");
	return;
#ifdef memtrace
