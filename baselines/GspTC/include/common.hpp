...
//TODO: 수정
inline void *AlignedRealloc(void *ptr, size_t oldSize, size_t newSize)
{
    if (ptr == NULL) {
        return AlignedMalloc(newSize);
    }

    void *newPtr = AlignedMalloc(newSize);
    if (newPtr == NULL) {
        printf("newPtr is NULL\n");
        return NULL;
    }

    // 원래 크기와 새로운 크기 중 작은 크기만큼 복사
